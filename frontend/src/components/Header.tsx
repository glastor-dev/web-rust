    ...
    <>
      <motion.header
        ...
        className={`fixed top-0 left-0 w-full z-50 transition-colors duration-300 ${
          isScrolled ? 'bg-[#050505]/80 backdrop-blur-md border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)]' : 'bg-transparent border-b border-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 h-20 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center gap-3 group" onClick={() => setMobileMenuOpen(false)}>
            <div className="w-10 h-10 flex items-center justify-center transition-transform duration-500 group-hover:scale-105">
              <img src="/images/isologo-copm.webp" alt="Glastor" className="w-full h-full object-contain" />
            </div>
            <div className="flex flex-col">
              <span className="font-display font-black text-xl tracking-tight text-white leading-none">GLASTOR</span>
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`absolute left-0 bottom-0 w-full h-[2px] bg-brand origin-left transition-transform duration-300 ${isActive ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`} />
                </Link>
              );
            })}
          </nav>
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Button asChild variant="default" size="sm" className="px-6 uppercase tracking-widest text-xs font-bold rounded-sm h-10 shadow-[0_0_15px_rgba(0,255,102,0.15)] hover:shadow-[0_0_25px_rgba(0,255,102,0.3)] transition-all">
              <Link to="/arquitectura">Configurar Arquitectura</Link>
            </Button>
          </div>
          ...
          >
            ...
            <nav className="flex flex-col gap-8 ml-4">
              ...
              <motion.div
                ...
                className="mt-12"
              >
                <Button asChild variant="default" size="lg" className="w-full h-16 text-lg uppercase tracking-widest" onClick={() => setMobileMenuOpen(false)}>
                  <Link to="/arquitectura">Configurar Arquitectura</Link>
                </Button>
              </motion.div>
            </nav>
